...
import com.example.testtaskpexelsapp.databinding.Fragment0ForTabLayoutCuratedImagesBinding
...
class Fragment0ForTabLayoutCuratedImages(private val context: MainActivity): Fragment(), IImagePressedListener {
    private lateinit var binding: Fragment0ForTabLayoutCuratedImagesBinding
    private val adapter = TabLayoutFragmentsAdapter(this, context)
    private val myViewModel: MyViewModel by activityViewModels()


    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        binding = Fragment0ForTabLayoutCuratedImagesBinding.inflate(inflater, container, false)
    ...
    private fun initRecyclerViewAdapter() = with(binding) {
        rcViewFrag0ForTabLayCuratedImages.layoutManager = GridLayoutManager(activity as AppCompatActivity, 2)
        rcViewFrag0ForTabLayCuratedImages.adapter = adapter
    }


    private fun initViewModel() {
        myViewModel.liveCuratedImagesData.observe(viewLifecycleOwner) {
