...
import com.example.testtaskpexelsapp.databinding.Fragment1ForTabLayoutBinding
...
class Fragment1ForTabLayout(private val context: MainActivity): Fragment(), IImagePressedListener {
    private lateinit var binding: Fragment1ForTabLayoutBinding
    private val adapter = TabLayoutFragmentsAdapter(this, context)
    private val myViewModel: MyViewModel by activityViewModels()


    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        binding = Fragment1ForTabLayoutBinding.inflate(inflater, container, false)
    ...
    private fun initRecyclerViewAdapter() = with(binding) {
        rcViewFrag1ForTabLay.layoutManager = GridLayoutManager(activity as AppCompatActivity, 2)
        rcViewFrag1ForTabLay.adapter = adapter
    }


    private fun initViewModel() {
        myViewModel.liveFragment1ImageCategoryData.observe(viewLifecycleOwner) {
