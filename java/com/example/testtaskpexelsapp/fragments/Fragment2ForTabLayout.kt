...
import com.example.testtaskpexelsapp.databinding.Fragment2ForTabLayoutBinding
...
class Fragment2ForTabLayout(private val context: MainActivity): Fragment(), IImagePressedListener {
    private lateinit var binding: Fragment2ForTabLayoutBinding
    private val adapter = TabLayoutFragmentsAdapter(this, context)
    private val myViewModel: MyViewModel by activityViewModels()


    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        binding = Fragment2ForTabLayoutBinding.inflate(inflater, container, false)
    ...
    private fun initRecyclerViewAdapter() = with(binding) {
        rcViewFrag2ForTabLay.layoutManager = GridLayoutManager(activity as AppCompatActivity, 2)
        binding.rcViewFrag2ForTabLay.adapter = adapter
    }


    private fun initViewModel() {
        myViewModel.liveFragment2ImageCategoryData.observe(viewLifecycleOwner) {
