...
import com.example.testtaskpexelsapp.databinding.Fragment5ForTabLayoutBinding
...
class Fragment5ForTabLayout(private val context: MainActivity): Fragment(), IImagePressedListener {
    private lateinit var binding: Fragment5ForTabLayoutBinding
    private val adapter = TabLayoutFragmentsAdapter(this, context)
    private val myViewModel: MyViewModel by activityViewModels()


    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        binding = Fragment5ForTabLayoutBinding.inflate(inflater, container, false)
    ...
    private fun initRecyclerViewAdapter() = with(binding) {
        rcViewFrag5ForTabLay.layoutManager = GridLayoutManager(activity as AppCompatActivity, 2)
        rcViewFrag5ForTabLay.adapter = adapter
    }


    private fun initViewModel() {
        myViewModel.liveFragment5ImageCategoryData.observe(viewLifecycleOwner) {
