...
import com.example.testtaskpexelsapp.databinding.Fragment6ForTabLayoutBinding
...
class Fragment6ForTabLayout(private val context: MainActivity): Fragment(), IImagePressedListener {
    private lateinit var binding: Fragment6ForTabLayoutBinding
    private val adapter = TabLayoutFragmentsAdapter(this, context)
    private val myViewModel: MyViewModel by activityViewModels()


    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        binding = Fragment6ForTabLayoutBinding.inflate(inflater, container, false)
    ...
    private fun initRecyclerViewAdapter() = with(binding) {
        rcViewFrag6ForTabLay.layoutManager = GridLayoutManager(activity as AppCompatActivity, 2)
        rcViewFrag6ForTabLay.adapter = adapter
    }


    private fun initViewModel() {
        myViewModel.liveFragment6ImageCategoryData.observe(viewLifecycleOwner) {
