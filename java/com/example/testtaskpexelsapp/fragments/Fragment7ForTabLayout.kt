...
import com.example.testtaskpexelsapp.databinding.Fragment7ForTabLayoutBinding
...
class Fragment7ForTabLayout(private val context: MainActivity): Fragment(), IImagePressedListener {
    private lateinit var binding: Fragment7ForTabLayoutBinding
    private val adapter = TabLayoutFragmentsAdapter(this, context)
    private val myViewModel: MyViewModel by activityViewModels()


    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        binding = Fragment7ForTabLayoutBinding.inflate(inflater, container, false)
    ...
    private fun initRecyclerViewAdapter() = with(binding) {
        rcViewFrag7ForTabLay.layoutManager = GridLayoutManager(activity as AppCompatActivity, 2)
        rcViewFrag7ForTabLay.adapter = adapter
    }


    private fun initViewModel() {
        myViewModel.liveFragment7ImageCategoryData.observe(viewLifecycleOwner) {
